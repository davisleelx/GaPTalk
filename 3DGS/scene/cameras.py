...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image=None, gt_alpha_mask=None, image_name=None, uid=None,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", depth=None,
                 exp_coeffs=None, mask=None,
                 ):
        ...
        self.exp_coeffs = torch.Tensor(exp_coeffs).to(self.data_device)
        self.mask = torch.from_numpy(mask).to(self.data_device) if mask is not None else None
        ...
        self.world_view_transform = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1).to(
            self.data_device) # (4, 4)
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx,
                                                     fovY=self.FoVy).transpose(0, 1).to(self.data_device) # (4, 4)
        self.full_proj_transform = (
            self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0) # (4, 4)
