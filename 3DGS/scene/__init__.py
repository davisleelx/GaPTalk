...
from scene.p2p_model import GeneratorModel, DiscriminatorModel 
...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], name="iteration"):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background, args.eval, read_image=args.read_image)
        ...
        if self.loaded_iter:
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                 "point_cloud",
                                                 f"{name}_" + str(self.loaded_iter),
                                                 "point_cloud.ply"),
                                    og_number_points=len(scene_info.point_cloud.points))
        else:
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent)

    def save(self, iteration, name="iteration"):
