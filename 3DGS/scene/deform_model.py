...
from gaussian_splatting.utils.time_utils import DeformNetwork
import os
...
class DeformModel:
    ...
    def save_weights(self, model_path, iteration, name="iteration"):
        out_weights_path = os.path.join(model_path, "deform/{}_{}".format(name, iteration))
        os.makedirs(out_weights_path, exist_ok=True)
        torch.save(self.deform.state_dict(), os.path.join(out_weights_path, 'deform.pth'))

    def load_weights(self, model_path, iteration=-1, name="iteration"):
        if iteration == -1:
            loaded_iter = searchForMaxIteration(os.path.join(model_path, "deform"))
        else:
            loaded_iter = iteration
        weights_path = os.path.join(model_path, "deform/{}_{}/deform.pth".format(name, loaded_iter))
