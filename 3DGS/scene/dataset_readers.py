...
from gaussian_splatting.utils.camera_utils import camera_nerfies_from_JSON
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png",
    ...
    lut[1:14] = 1
    # lut[17] = 2 # 头发
    lut_mouth_area = np.zeros((256, ), dtype=np.uint8)
    lut_mouth_area[11:14] = 1
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        choose_video = contents['choose_video']
        total = len(frames)
        for idx, frame in enumerate(frames):
            if idx > 650:
                break
        # for idx in np.linspace(0, total-1, num=min(1000, total)):
            frame = frames[int(idx)]
            sub_idx = frame['sub_idx']
            # cam_name = os.path.join(path, frame["file_path"] + extension)
            cam_name = os.path.join(path, choose_video, 'videos/crop', f'{sub_idx:06d}.png')

            # matrix = np.linalg.inv(np.array(frame["transform_matrix"])) # (4, 4)
            # R = -np.transpose(matrix[:3, :3]) # (3, 3)
            # R[:, 0] = -R[:, 0]
            # T = -matrix[:3, 3] # (3)

            # NeRF 'transform_matrix' is a camera-to-world transform            
            c2w = np.array(frame["transform_matrix"]) # (4, 4)
            ...
            # image_path = frame["file_path"] + extension
            image_path = os.path.join(choose_video, 'videos/crop', f'{sub_idx:06d}.png')
            ...
            if read_image:
                ...
                mask_path = os.path.join(choose_video, 'videos/parsing_crop', f'{sub_idx:06d}.png')
                ...
                res[res != 0] = 255
                mask_img = res
                arr[mask_img < 0.5] = 1. if white_background else 0.

                cv2.LUT(res_mouth_area, lut_mouth_area, res_mouth_area)
                ...
                face_mask_path = os.path.join(choose_video, 'videos/face_mask_crop', f'{sub_idx:06d}.png')
                rest_mask = cv2.imread(face_mask_path)
                rest_mask = rest_mask.transpose((2, 0, 1)) / 255.

            # image_path = os.path.join(choose_video, 'videos', f'{sub_idx:06d}.jpg')
            # image_name = Path(cam_name).stem
            # if read_image:
            #     image = Image.open(image_path)
            ...
            #     mask_path = os.path.join(choose_video, 'videos/parsing', f'{sub_idx:06d}.png')
            #     mask_img = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED).astype(np.uint8)
            #     res = mask_img.astype(np.uint8)
            #     res_mouth_area = res.copy()
            #     cv2.LUT(res, lut, res)
            ...
            #     face_mask_path = os.path.join(choose_video, 'videos/face_mask', f'{sub_idx:06d}.png')
            #     rest_mask = cv2.imread(face_mask_path)
            #     rest_mask = rest_mask.transpose((2, 0, 1)) / 255.
            ...
            exp_coeffs = np.array(frame['base_code'][80:144])

            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image if read_image else None,
                                        image_path=image_path, image_name=image_name, width=image.size[0] if read_image else 512,
                                        height=image.size[1] if read_image else 512,
                                        exp_coeffs=exp_coeffs, 
                                        # mask=rest_mask if read_image else None,
                                        mask=mask_img_mouth_area if read_image else None,
                                        ))

    return cam_infos


def readNerfSyntheticInfo(path, white_background, eval, extension=".png", read_image=True):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, extension, read_image)
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        path, "transforms_test.json", white_background, extension, read_image)
