...
def put_text_alignmentcenter(img, img_size, text_str, color, offset_x):
    ...
    img = cv2.putText(img.copy(), text_str, (textX, textY), font, 1, color, 2)
...
def load_state_dict(model, state_dict):
    ...
    for (k, v) in state.items():
        if k not in to_load:
            ...
            if state[k].shape == to_load[k].shape:
                state[k] = to_load[k]
            else:
                print(f"    - WARNING: Model file contains key {k} ({list(v.shape)}), but to load key {k} ({list(to_load[k].shape)})")
