...
from basicsr.archs.arch_util import ResidualBlockNoBN, flow_warp, make_layer, default_init_weights
...
class StyleGAN2GeneratorCSFT(StyleGAN2GeneratorClean):
    ...
    def forward(self,
        ...
        # noises
        if noise is None:
            if randomize_noise is True:
                noise = [None] * self.num_layers  # for each style conv layer
            elif randomize_noise == 'zero':
                noise = [torch.zeros_like(getattr(self.noises, f'noise{i}')) for i in range(self.num_layers)]
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(self.style_convs[::2],
                                                        self.style_convs[1::2], noise[1::2], noise[2::2], self.to_rgbs):
...
class ConvReLUConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mode=None, zero_out_init=False):
...
class GFPGANv1Clean_UnetWindow(nn.Module):
    ...
    def __init__(
        ...
        self.zero_out_init = zero_out_init
        ...
        # self.conv_body_first = nn.Conv2d(6, channels[f'{first_out_size}'], 1)
        ...
        for i in range(3, self.log_size + 1):
            ...
            if temp_type == 'recurrent':
                ...
                pass
                # self.recurrent_forward.append(ConvReLUConv(sft_out_channels*3, sft_out_channels,zero_out_init=zero_out_init))
            elif self.temp_type == 'none':
                pass

        if GFPGANv1_load_path is not None:
    ...
    def forward(self, x, return_latents=False, return_rgb=True, randomize_noise=True, interpolate=False):
        ...
        for i in range(self.log_size - 2):
            ...
            feat = self.unet_window_down(feat, i)
