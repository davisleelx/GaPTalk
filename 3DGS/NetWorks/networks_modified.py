...
def define_G(input_nc, output_nc, ngf, netG, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1, 
             n_blocks_local=3, norm='instance', gpu_ids=[], final=nn.Tanh(), opt=None, **kwargs):    
    norm_layer = get_norm_layer(norm_type=norm)
    if netG == 'global':    
        netG = GlobalGenerator(input_nc, output_nc, ngf, n_downsample_global, n_blocks_global, norm_layer)       
    elif netG == 'local':        
        netG = LocalEnhancer(input_nc, output_nc, ngf, n_downsample_global, n_blocks_global,
                             n_local_enhancers, n_blocks_local, norm_layer, final=final)
    elif netG == 'encoder':
...
class LocalEnhancer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3, n_blocks_global=9, 
        ...
        model_y = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf_global, kernel_size=7, padding=0), norm_layer(ngf), activation]
        ### downsample
        for i in range(n_downsample_global):
            mult = 2**i
            model_y += [nn.Conv2d(ngf_global * mult, ngf_global * mult * 2, kernel_size=3, stride=2, padding=1),
                    norm_layer(ngf_global * mult * 2), activation]
        self.model_y_before = nn.Sequential(*model_y)
        ...
        for n in range(1, n_local_enhancers+1):
            ...
            model_y_downsample = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf_global, kernel_size=7, padding=0), 
                                norm_layer(ngf_global), nn.ReLU(True),
                                nn.Conv2d(ngf_global, ngf_global * 2, kernel_size=3, stride=2, padding=1), 
                                norm_layer(ngf_global * 2), nn.ReLU(True)]
            ...
            setattr(self, 'model'+str(n)+'_3', nn.Sequential(*model_y_downsample))
        
        self.downsample = nn.AvgPool2d(3, stride=2, padding=[1, 1], count_include_pad=False)


    # 20180929: change input style
    def forward(self, input, middle=None):
        ### create input pyramid
        input_downsampled = [input]
        middle_downsampled = [middle]
        # input_downsampled = [torch.cat([input, params.unsqueeze(2).unsqueeze(3).repeat(1, 1, 512, 512)], dim=1)]
        for i in range(self.n_local_enhancers):
            input_downsampled.append(self.downsample(input_downsampled[-1]))
            middle_downsampled.append(self.downsample(middle_downsampled[-1]))

        ### output at coarest level
        output_prev = self.model(self.model_before(input_downsampled[-1]), self.model_y_before(middle_downsampled[-1]))
        ### build up one layer at a time
        for n_local_enhancers in range(1, self.n_local_enhancers+1):
            model_downsample = getattr(self, 'model'+str(n_local_enhancers)+'_1')
            model_upsample = getattr(self, 'model'+str(n_local_enhancers)+'_2')
            model_y_downsample = getattr(self, 'model'+str(n_local_enhancers)+'_3')
            input_i = input_downsampled[self.n_local_enhancers-n_local_enhancers]
            middle_i = middle_downsampled[self.n_local_enhancers-n_local_enhancers]
            output_prev = model_upsample(model_downsample(input_i) + output_prev + model_y_downsample(middle_i))
        return output_prev
...
class GlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d, 
        ...
        self.merge = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1),
                    norm_layer(512), activation)
