...
from gaussian_splatting.utils.rigid_utils import exp_se3
...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False):
        ...
        self.embed_time_fn, time_input_ch = get_embedder(self.t_multires, 64)
    ...
    def forward(self, x, t):
        t_emb = self.embed_time_fn(t) # (, 21)
        if self.is_blender:
            t_emb = self.timenet(t_emb) # better for D-NeRF Dataset
        x_emb = self.embed_fn(x) # (, 63)
