...
from gaussian_splatting.utils.loss_utils import l1_loss, l2_loss, ssim, kl_divergence, VGGPerceptualLoss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        exp_coeffs = viewpoint_cam.exp_coeffs
        ...
        if iteration < opt.warm_up:
            ...
            d_xyz, d_rotation, d_scaling = deform.step(gaussians.get_xyz.detach(), exp_coeffs_input + ast_noise)
        ...
        image_mouth = image * viewpoint_cam.mask
        gt_image_mouth = gt_image * viewpoint_cam.mask
        # cv2.imwrite(f"{args.model_path}/mask.jpg", (viewpoint_cam.mask.detach().cpu().numpy() * 255.).astype(np.uint8))
        # cv2.imwrite(f"{args.model_path}/gt_mouth.jpg", (gt_image_mouth.permute(1, 2, 0).detach().cpu().numpy()[:, :, ::-1] * 255.).astype(np.uint8))
        # cv2.imwrite(f"{args.model_path}/gt.jpg", (gt_image.permute(1, 2, 0).detach().cpu().numpy()[:, :, ::-1] * 255.).astype(np.uint8))
        ...
        # loss = Ll1
        # if iteration > 10000:
        # loss += 0.01 * vgg_loss_func(image.unsqueeze(0), gt_image.unsqueeze(0))
