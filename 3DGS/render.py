...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    # for idx, view in enumerate(views):
        ...
        torchvision.utils.save_image(rendering + gt, os.path.join(debug_path, '{0:05d}'.format(idx) + ".png"))
        # torchvision.utils.save_image(depth, os.path.join(depth_path, '{0:05d}'.format(idx) + ".png"))
    
    # end = time.time()
    # print("Points: ", len(gaussians.get_xyz))
    # print("Frames: {}, Time: {}, FPS: {}".format(len(views), end - start, len(views) / (end - start)))
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
    ...
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, name=name)
        deform = DeformModel(dataset.is_blender, dataset.is_6dof)
        # deform.load_weights(dataset.model_path)
        deform.load_weights(dataset.model_path, name=name, iteration=scene.loaded_iter)
