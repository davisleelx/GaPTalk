...
from data.dataset import Dataset, DatasetValid
...
def log_validation(
    ...
    for idx in sample_idx:
        ...
        pose_image_pil = Image.fromarray(sample['img_crop_mask']).convert("RGB")
        gt_image_pil = Image.fromarray(sample['img_crop']).convert("RGB")
        ...
        ref_pose_pil = ref_pose_pil.resize((w, h))
        pose_image_pil = pose_image_pil.resize((w, h))
        gt_image_pil = gt_image_pil.resize((w, h))

        canvas.paste(ref_image_pil, (0, 0))
        canvas.paste(ref_pose_pil, (w, 0))
        canvas.paste(pose_image_pil, (w * 2, 0))
        canvas.paste(res_image_pil, (w * 3, 0))
        canvas.paste(gt_image_pil, (w * 4, 0))
...
def main(cfg):
    ...
    train_dataset = Dataset(cfg.data, training=True)
    valid_dataset = DatasetValid(cfg.data, training=False)
