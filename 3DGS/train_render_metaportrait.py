...
from gaussian_splatting.models.metaportrait.models.metaportrait_video_model import MetaportraitVideoRecurrentModel
...
class FittingImage(object):
    def __init__(self, dataset, pipe, opt, gpu_id) -> None:
        ...
        netG = GFPGANv1Clean_UnetWindow(**opt.netG.network_g)
        load_state_dict(netG, torch.load(opt.netG.path.pretrain_network_g, map_location=torch.device("cpu"))['params'])
    ...
    def load_data(self, batch):
        ...
        self.img_tensor = batch['img'].to(self.device)
        self.mask_img_tensor = batch['mask_img'].to(self.device)
    ...
    def perform_fitting(self, batch):
        B, L, C, H, W = self.img_tensor.shape
        gt_img = (self.img_tensor[0, 0].detach().cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
        # gt_img = ((self.img_tensor[0, 0].detach().cpu().permute(1, 2, 0).numpy() + 1) / 2 * 255).astype(np.uint8)
        ...
        with torch.no_grad():
            ...
            for i in range(L):
                exp_coeffs = batch['base_code'][0, i, 80:144].squeeze().cuda() # (64)
                viewpoint_cam = Camera(colmap_id=0, R=(batch['cam_info']['batch_c2w_Rmats'][0, i].squeeze()).cpu().numpy(), T=(batch['cam_info']['batch_w2c_Tvecs'][0, i].squeeze()).cpu().numpy(),
                    FoVx=batch['cam_info']['FoVx'][0], FoVy=batch['cam_info']['FoVy'][0],
        ...
        x = self.mask_img_tensor
        y = torch.stack(render_image, dim=0).unsqueeze(0)
        # pred = self.netG(x)
        pred = self.netG(torch.cat([x, y], dim=2))
        # pred = self.netG(x, y)

        pred_dict = {'image': pred[0]}
        batch_loss_dict = self.loss_utils.calc_total_loss(
            pred_dict=pred_dict, 
            gt_rgb=self.img_tensor[0], mask_tensor=self.mask_tensor[0], netD=self.netD,
        )
